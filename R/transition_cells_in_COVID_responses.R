...
transition_cells<-subset(data,celltype=='B'& pearson>0.22) %>% colnames()
ctrl<-subset(data,celltype=='B' & pearson<0.22) %>% colnames()
...
# classical monocytes
pdf('classical_monocyte_pearson_dist.pdf')
pearson_sub<-subset(data,celltype=='Classical monocyte')$pearson
fit <- Mclust(pearson_sub,G=2)
plot(density(pearson_sub),main="Transition index distribution for classical monocyte")
rug(jitter(pearson_sub[fit$classification==2 & fit$z[,2]>0.8]),col='red')
...
monocyte$transition[names(pearson_sub[fit$classification==2 & fit$z[,2]>0.8])]<-'transition'
monocyte$transition[names(pearson_sub[fit$classification==1 & fit$z[,1]>0.8])]<-'stable'
...
# platelets
stats=data@meta.data %>% filter(celltype=='Platelet',pearson>0.5) %>% group_by(treatment,hamster) %>% summarize(n=n()) %>% left_join(data@meta.data %>%
    group_by(treatment,hamster) %>% summarize(n_t=n())) %>% mutate(per=n/n_t) %>% group_by(treatment) %>% 
    mutate(mean_n=mean(n),mean_per=mean(per),sd_n=sd(n),sd_per=sd(per))
# A tibble: 9 x 9
# Groups:   treatment [5]
